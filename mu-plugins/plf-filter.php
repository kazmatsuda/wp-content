...
class Plf_filter {    
        ...
        foreach (array('url_path', 'url_q_and', 'url_q_not') as $item) {
            ...
            if(empty($ar_key)) {
                ...
                foreach ($ar_key as $key) {
                    if(preg_match("#{$reg_before}{$key}{$reg_after}#ui", $_url[$item])){
                        $match[$item] += 1;
                    }
                }
            }
    ...
    static function plf_filter( $option, $default = false) {
        if ( defined( 'WP_SETUP_CONFIG' ) || did_action( 'muplugins_loaded' ) === 0)
            return false;
        ...
        foreach ( $act_plugins as $item ) {
            ...
            if(!$unload) {
                if($option === 'active_sitewide_plugins') {
                    $new_plugins[$item] = $opt_value[$item];
                } else {
                    $new_plugins[] = $item;
                }
            }
